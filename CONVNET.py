...
def exe_bn_layer(tensor, layer_o, net_info, l_index, is_first, is_training, trainable, act_o):
    ...
    pars = []
    for i in range(act_o['size']):
        offset = tf.get_variable(name=PARAMETERS_NAME[p_index] % (l_index, i), shape=shape,
                                 initializer=tf.constant_initializer(0.0), trainable=offset_trainable)
        scale = tf.get_variable(name=PARAMETERS_NAME[p_index + 1] % (l_index, i), shape=shape,
                                initializer=tf.constant_initializer(1.0), trainable=scale_trainable)
        mv_mean = tf.get_variable(name=PARAMETERS_NAME[p_index + 2] % (l_index, i), shape=shape,
                                  initializer=tf.constant_initializer(0.0), trainable=False)
        mv_var = tf.get_variable(name=PARAMETERS_NAME[p_index + 3] % (l_index, i), shape=shape,
